...
generated_paths=(
    ...
    examples/shape-matrix/REPORT.md
)
...
# The shape matrix is generated by RUNNING its binary, not by a build script:
# it drives both generators over synthesized fixtures. Its report is committed
# for the same reason the bindings are — the generators decide legality AND
# produce the report, so "the build fails until new cells are classified" would
# be vacuous on its own. A committed report makes every changed answer a
# reviewed diff. Its own stdout is the generators' build-script chatter.
echo "== regenerating the shape matrix (cargo run)"
cargo run --release -q -p shape-matrix >/dev/null
