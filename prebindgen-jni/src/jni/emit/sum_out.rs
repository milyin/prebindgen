...
//! ([`apply_sum_returns`](unfold::apply_sum_returns)); this
...
pub(crate) fn encode_sum_group(
    ...
    let (source, sum) = unfold::DeliveryBridge::sum(context, tag_leaf);
    ...
    let arg_exprs: Vec<TokenStream> = leaves
        .iter()
        .enumerate()
        .map(|(idx, leaf)| unfold::DeliveryBridge::argument(context, leaf, &obj_idents[idx]))
        .collect();
...
///
/// [`unfold::segment`] supplies the gate — one tuple bind
...
pub(crate) fn encode_presence_group(
    ...
    let inner = unfold::segments_at(leaves, leaves[0].groups.len() + 1);
    let tail_of = |leaf: &crate::jni::compile::OutWire| -> Vec<unfold::PathStep> {
        leaf.reach.iter().skip(consumed).cloned().collect()
    };
    for seg in &inner {
        let seg_args = std::cell::RefCell::new(Vec::new());
        let place = unfold::LeafPlace {
            base: matched.clone(),
            base_is_ref: true,
    ...
    for (index, leaf) in leaves.iter().enumerate().skip(1) {
        ...
        let tail: Vec<unfold::PathStep> = leaf.reach.iter().skip(consumed).cloned().collect();
