...
fn shape(recipe: &Recipe) -> String {
    match recipe {
        Recipe::Constructing(s) => format!("{:?}", Named(s)),
        Recipe::Deconstructing(s) => format!("{:?}", Named(s)),
    }
}
...
fn a_tagged_union_is_its_arms() {
    ...
    // arm is live is the adapter's business, so no recipe mentions it.
    let model = model();
    let gen = Cbindgen::builder().tagged_union(syn::parse_quote!(Reply));
    assert_eq!(
        recipes_of(&gen, &model, "Reply"),
...
fn a_value_read_two_ways_inside_a_struct_has_two_rows() {
    ...
    // than they do on their own, which is two recipes of one crossing with the
...
/// recipe while `bindings` still asked every string field for it.
#[test]
fn a_string_declared_as_a_handle_keeps_the_field_reading() {
    ...
    let recipes = gen
        .recipes(&model)
        .expect("a String handle declaration leaves the field recipe in place");
    gen.bindings(&model, &recipes)
        .expect("every string field still finds the field recipe");
}
