...
//! with [`fun!`](crate::fun) / [`ptr_type!`](crate::ptr_type) /
//! [`data_type!`](crate::data_type) / [`enum_type!`](crate::enum_type) /
//! [`tagged_union!`](crate::tagged_union). The C name of a declared
//! type's generated destructor can be pinned by chaining [`base_name`](crate::PtrTypeDecl::base_name).
//!
//! ## C ABI conventions
//!
//! * **Pointer struct** (declared with [`ptr_type!`](crate::ptr_type)): a `Box`-owned
...
//!   per handle.
//! * **Data struct** (declared with [`data_type!`](crate::data_type)): a by-value
...
//! * **Enum type** (declared with [`enum_type!`](crate::enum_type)): a fieldless enum,
...
//! error type via [`data_type!`](crate::data_type) + [`error`](crate::DataTypeDecl::error) — otherwise the
...
struct TypeCfg {
    ...
    /// `mangle_rust_type`-derived base. Set by [`base_name`](crate::PtrTypeDecl::base_name). `None` ⇒
...
/// impl). See [`value_type!`](crate::value_type) / [`value_type!`](crate::value_type).
...
struct ValueOpaqueCfg {
    ...
    /// [`repr_c_type!`](crate::repr_c_type); `false` for `opaque_data_struct` /
    ...
    /// instance 3). Set by [`assume_field_validity`](crate::ReprCTypeDecl::assume_field_validity). See
...
struct CbCfg {
    ...
    /// [`base_name`](crate::PtrTypeDecl::base_name). `None` ⇒ bases come from the arguments.
    base: Option<String>,
    ...
    /// [`takeable_param`](crate::CallbackDecl::takeable_param); each such arg type must be an inline-opaque
    /// type ([`value_type!`](crate::value_type) / [`value_type!`](crate::value_type)).
...
/// The engine that produced a [`Cbindgen`], and everything it produced.
///
...
pub(crate) enum Engine {
    ...
    #[cfg(feature = "v2")]
    V2(Box<prebindgen_registry_v2::Generation<v2::CTarget>>),
}
...
impl Cbindgen {
    ...
    pub fn registry(&self) -> &prebindgen_registry::Registry {
        match &self.engine {
            ...
            #[cfg(feature = "v2")]
            Engine::V2(_) => panic!(
                "`Cbindgen::registry` is the v1 engine's resolution; this binding was \
                 generated by v2. Read `Cbindgen::skipped()` instead"
            ),
        }
    }
    ...
    /// What this binding asked for and did not get.
    ///
    /// Empty under v1: its answer is "everything declared, or the build
    /// failed", so it leaves nothing out to account for. Under v2, one entry
    /// per declaration a missing capability stopped — see
    /// [`prebindgen_registry_v2::Generation::skipped`].
    #[cfg(feature = "v2")]
    pub fn skipped(
        &self,
    ) -> &[(
        prebindgen_registry_v2::Declaration,
        prebindgen_registry_v2::Skip,
    )] {
        match &self.engine {
            Engine::V1(_) => &[],
            Engine::V2(generation) => generation.skipped(),
        }
    }

    /// Print what was skipped as cargo warnings, one line per capability.
    ///
    /// Does nothing under an engine that skips nothing, so a build script can
    /// call it unconditionally. It cannot ask `cfg(feature = "v2")`: cargo
    /// passes a package's features to a build script as environment
    /// variables, not as cfgs.
    pub fn warn_skipped(&self) {
        #[cfg(feature = "v2")]
        if let Engine::V2(generation) = &self.engine {
            crate::v2::warn_skipped(generation.skipped());
        }
    }
...
pub struct CbindgenBuilder {
    ...
    /// `union`. Declared with [`tagged_union!`](crate::tagged_union).
    ...
    /// a declaration's `base_name` replaces the *base* token fed to these
...
/// The v2 engine, so a build script can name what [`Cbindgen::skipped`] hands
