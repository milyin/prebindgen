...
impl Cbindgen {
        ...
        match &self.engine {
            ...
            Engine::V2(_) => panic!(
                "`Cbindgen::registry` is the v1 engine's resolution; this binding was \
                 generated by v2. Read `Cbindgen::report()` instead"
            ),
        }
    }
    ...
    /// failed", so it has no partial surface to report. Producing one for v1
    /// too is #719 §4 and follows this change.
    #[cfg(feature = "v2")]
    pub fn report(&self) -> Option<&prebindgen_registry_v2::Report> {
        match &self.engine {
            Engine::V1(_) => None,
    ...
    /// Write the report into `dir`, and print its summary as cargo warnings.
    ///
    /// Returns the paths written — empty under an engine that produces no
    /// report, so a build script can call this unconditionally. It cannot ask
