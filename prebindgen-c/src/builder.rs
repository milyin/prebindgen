...
impl CbindgenBuilder {
    ...
    /// gravestone write-back and no [`prebindgen::core::Gravestone`] impl are needed** —
    /// only the autogenerated [`prebindgen::core::Transmute`] glue (emitted here) plus a
    ...
    /// via [`prebindgen::core::Transmute`] and write a **gravestone** back on consume
    /// (safe drop-after-move), exposing an `Option<rust_ty>` null niche. The
    /// consumer must implement [`prebindgen::core::Gravestone`] for `opaque_ty` — only
    ...
    /// [`prebindgen::core::Transmute`] and a `&T` borrow / `impl Fn(&T)` callback as a
