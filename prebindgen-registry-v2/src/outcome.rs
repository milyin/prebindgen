...
pub struct Skip {
    ...
    /// Root → failing site, one step per line of the printed path: the declaration,
...
impl Skip {
    /// A skip at the declaration itself, with no dependency to walk through.
    pub fn direct(
...
pub enum EngineError {
    ...
    /// Declared declarations that match no captured `#[prebindgen]` item. A
        ...
        match self {
            ...
            EngineError::DeclaredNotFound { entries } => {
                ...
                for declaration in entries {
                    writeln!(
                        f,
                        "  {declaration} — no captured {} `{}`",
                        declaration.describe_captured(),
                        declaration.name()
                    )?;
                }
                write!(
