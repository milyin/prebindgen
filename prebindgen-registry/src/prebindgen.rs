...
//!
//! One method per `#[prebindgen]` item kind an adapter still emits per item
//! (`on_struct`, `on_variant`, `on_enum`, `on_const`) returning typed Rust
//! items to emit, plus the items they depend on (`prerequisites`) and two
//! invariant checks. An exported wrapper is not among them: it is an artifact
//! of the adapter's frozen assembly.
//!
...
/// The file emitter calls `on_struct` / `on_variant` / `on_enum` / `on_const`
/// to produce the per-item output, plus `prerequisites` and the two
/// `validate` hooks for adapter invariants. An exported wrapper is not among
/// them: it is an artifact of the adapter's frozen assembly.
///
...
pub trait Prebindgen {
    ...
    // the pipeline (#275). Final Rust source types are generated by `RustWriter` from
    // those model facts; retained type syntax is unavailable here.
