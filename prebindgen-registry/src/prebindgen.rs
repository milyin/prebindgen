...
//! items they depend on (`prerequisites`) and two invariant checks.
//!
...
/// to produce the per-item wrapper code, plus `prerequisites` and the two
...
/// by [`Emit`](crate::Emit), never a typed view of retained syntax. It is silent
...
pub trait Prebindgen {
    ...
    // the pipeline (#275). Final Rust source types are generated by `Emit` from
