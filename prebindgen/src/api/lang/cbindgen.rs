...
mod tests {
    ...
    fn opaque_owned_transmute_by_value() {
        ...
        assert!(
            compact.contains(
                "size_of::<zenoh_flat::Payload>()==::core::mem::size_of::<OpaquePayload>()"
            ),
            "{src}"
        );
        assert!(
        ...
        assert!(
            compact.contains(
                "<OpaquePayloadas::prebindgen::Transmute>::into_rust(::core::ptr::read(v),)"
            ),
            "{src}"
        );
        assert!(
            compact
                .contains("ptr::write(v,<OpaquePayloadas::prebindgen::Gravestone>::gravestone())"),
            "{src}"
        );
    ...
    fn opaque_data_no_gravestone_writeback() {
        ...
        assert!(
            compact
                .contains("align_of::<zenoh_flat::Stamp>()==::core::mem::align_of::<z_stamp_t>()"),
            "{src}"
        );
        // Consume moves the value out by transmute…
        assert!(
            compact
                .contains("<z_stamp_tas::prebindgen::Transmute>::into_rust(::core::ptr::read(v)"),
            "{src}"
        );
    ...
    fn takeable_callback_param() {
        ...
        assert!(
            compact.contains(
                "pubunsafeextern\"C\"fnsample_take(dst:*mutz_sample_t,src:*mutz_sample_t)"
            ),
            "{src}"
        );
        assert!(
            compact
                .contains("ptr::write(src,<z_sample_tas::prebindgen::Gravestone>::gravestone())"),
            "{src}"
        );
    }
