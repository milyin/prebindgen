...
/// impl). See [`Cbindgen::opaque_data`] / [`Cbindgen::opaque_owned`].
...
struct CbCfg {
    ...
    /// type ([`Cbindgen::opaque_owned`] / [`Cbindgen::opaque_data`]).
...
impl Cbindgen {
    ...
    /// [`Self::opaque_owned`] for types owning external data.
    pub fn opaque_data(self, rust_ty: syn::Type, opaque_ty: syn::Type) -> Self {
        self.declare_opaque(rust_ty, opaque_ty, OpaqueKind::Data)
    }

    /// Declare an **inline-opaque by-value, owns-external-data** type: like
    /// [`Self::opaque_data`], but for a Rust value that owns external resources
    ...
    pub fn opaque_owned(self, rust_ty: syn::Type, opaque_ty: syn::Type) -> Self {
        self.declare_opaque(rust_ty, opaque_ty, OpaqueKind::Owned)
    }

    /// Shared body of [`Self::opaque_data`] / [`Self::opaque_owned`].
    fn declare_opaque(mut self, rust_ty: syn::Type, opaque_ty: syn::Type, kind: OpaqueKind) -> Self {
        let method = match kind {
            OpaqueKind::Data => "opaque_data",
            OpaqueKind::Owned => "opaque_owned",
        };
    ...
    /// arg type must be an inline-opaque type ([`Self::opaque_owned`] /
    /// [`Self::opaque_data`]). Chain after `.callback(...)` (and any `.name(...)`).
    ...
    /// Whether a declared inline-opaque type owns external data (needs a gravestone
    /// write-back on consume) or is plain data.
    fn opaque_kind(&self, ty: &syn::Type) -> Option<OpaqueKind> {
        self.value_opaque
            .get(&TypeKey::from_type(ty))
            .map(|c| c.kind)
    }
...
impl Prebindgen for Cbindgen {
    ...
    fn prerequisites(&self, registry: &Registry<()>) -> Vec<syn::Item> {
        ...
        // the live Rust value in place; NULL/gravestone ⇒ no-op).
        {
            ...
            for (key, cfg) in vo {
                ...
                // an `opaque_owned` type, leave `src` a gravestone (so the
                // trampoline's post-call drop is a no-op); an `opaque_data` type owns
    ...
    fn on_input_type_rank_0(&self, ty: &syn::Type, _r: &Registry<()>) -> Option<ConverterImpl<()>> {
        ...
        // transmute (move). For an `opaque_owned` type, write a gravestone back so a
        // later `_drop` is a no-op (safe drop-after-move); an `opaque_data` type
...
mod tests {
    ...
    /// An `opaque_owned` type is passed BY VALUE via transmute to/from an opaque
    ...
    fn opaque_owned_transmute_by_value() {
        ...
        let cbindgen = Cbindgen::new()
            .source_module(syn::parse_quote!(zenoh_flat))
            .opaque_owned(syn::parse_quote!(Payload), syn::parse_quote!(OpaquePayload))
        ...
        assert!(
            compact.contains("<OpaquePayloadas::prebindgen::Transmute>::into_rust(::core::ptr::read(v),)"),
            "{src}"
        );
        assert!(
    ...
    /// An `opaque_data` (plain-data) type is passed BY VALUE like `opaque_owned`,
    ...
    fn opaque_data_no_gravestone_writeback() {
        ...
        let cbindgen = Cbindgen::new()
            .source_module(syn::parse_quote!(zenoh_flat))
            .opaque_data(syn::parse_quote!(Stamp), syn::parse_quote!(z_stamp_t))
            .base_name("z_stamp_t")
            .function(syn::parse_quote!(z_stamp_make))
            .function(syn::parse_quote!(z_stamp_take))
            .panic();

        let src = write(&cbindgen, &mut registry, "opaque_data");
        let compact: String = src.split_whitespace().collect();
        ...
        assert!(
            ...
            "opaque_data must not reference Gravestone: {src}"
        );
    ...
    fn takeable_callback_param() {
        ...
        let cbindgen = Cbindgen::new()
            .source_module(syn::parse_quote!(zenoh_flat))
            .opaque_owned(syn::parse_quote!(Sample), syn::parse_quote!(z_sample_t))
