...
impl Cbindgen {
    ...
    /// elsewhere). This emits the [`crate::core::Transmute`] impl for `opaque_ty`
    /// (the unsafe rust<->opaque glue) plus a `const _` size+align equality assert
    /// (fail-closed); the converters move values via `Transmute` and write a
    /// gravestone back on consume (safe drop-after-move), exposing an
    /// `Option<rust_ty>` null niche. The consumer must implement
    /// [`crate::core::Gravestone`] for `opaque_ty` — only its *logic*
    /// (`rust_gravestone` / `rust_is_gravestone`).
