...
//! Up means a new classifier landed outside the frontend. Down means one was
//! migrated — the goal, but it still edits the ledger, so F3–F6's progress shows
//! as a diff instead of being invisible.
//!
...
///
/// Extending this list is F1's job — one line, plus a regenerated ledger whose
/// diff is the classification decision.
const WATCHED: &[&str] = &["Type", "Expr"];

/// Ledger location, relative to `src/`.
const LEDGER: &str = "api/core/frontend/boundary.ledger";

/// Regenerated verbatim on every write, so the contract cannot drift from the
/// numbers underneath it.
const HEADER: &str = "\
# prebindgen source-syntax boundary ledger — issue #211, stage F8.
#
# One line per file OUTSIDE `api/core/frontend/`, counting how many times a
...
# MEANS. #211 says only `core::frontend` may do that, so this file freezes the
...
#     git diff prebindgen/src/api/core/frontend/boundary.ledger
#
# A count going DOWN is the goal (a classifier moved into the frontend) and is
...
# writes a `syn::` path. Each is a candidate addition to WATCHED (see F1):
#
...
///
/// Excluded: this module's own directory (the frontend is where classification
...
fn scan_tree(src_root: &Path) -> BTreeMap<String, usize> {
    ...
    while let Some(dir) = stack.pop() {
        ...
        for entry in entries {
            ...
            if rel.starts_with("api/core/frontend/") {
                continue;
            }
...
fn boundary_ledger() {
    ...
    panic!(
        "BOUNDARY LEDGER DRIFT — source-syntax classification sites changed:\n\
         {drift}\n\
         A new classifier outside core::frontend needs one of:\n\
         \x20 * move it into core::frontend (see #211), or\n\
