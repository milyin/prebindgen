...
request fields. The frontend hands the engine a `Declaration` — here
`Declaration::Type(Stamp)` — paired with the [choice](../../stages/03-requests.md#what-a-choice-records) it recorded for it, and
records that same choice for the type in its own target, which is how a later
