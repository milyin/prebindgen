...
position: wherever this Stamp sits           // what the target looks its choice up by
offered:  [ Stamp.fields, atomic ]           // registered by the registry from Flat

held by the target, not passed in:
choice:   what this target recorded for Stamp
```
...
Stamp, IntoRust
  ...
  conversion: <what this target recorded for Stamp>   // with the key that choice makes
...
struct [choices](../../stages/03-requests.md#what-a-choice-records) these pages use — a C `data_struct`, a Kotlin data class — the
...
standing for the choice it found at that position — which is what later stages
...
- Selection precedes traversal. The target sees the type and the position it
  resolves its choice from, not the fields; a target answering `atomic` — an opaque handle — would leave
