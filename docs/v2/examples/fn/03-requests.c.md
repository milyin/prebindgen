...
```text
recorded in the binding by the C frontend, under `fn:stamp_sum` (C function):
...
- Without the `Stamp` declaration, the frontend records no rule for `Stamp`
  and no output exposing it. Planning finds no rule for the parameter and
  skips the function with `unsupported.conversion.no_rule`.
  [Retention][fn_retain] preserves that skip; it does not first discover the
  missing [conversion](../../stages/04-select.md#select-conversion-relations).
