...
- One aggregate in, one integer out: no synthetic parameters, no out-parameter.
  The symbol and the convention are the recorded [policy](../../stages/03-requests.md#what-policy-means)'s, [as the request cell shows][fn_requests_c].
...
- No route is declared because both [nodes](../../stages/04-values.md#plan-value-conversions) are infallible, [as planned][fn_values_c]. A later
