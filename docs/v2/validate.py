...
def validate(root):
    ...
    fields(manifest, ['version', 'languages', 'stages', 'examples', 'deferred_examples', 'cells', 'contracts'], 'manifest')
    require(type(manifest['version']) is int and manifest['version'] == 2, 'unsupported manifest version')
    for key in ['languages', 'stages', 'examples', 'deferred_examples', 'cells', 'contracts']:
    ...
    for contract in contracts:
        fields(contract, ['id', 'stage', 'title'], 'contract')
        text_id(contract['id'], 'contract')
        nonempty(contract['title'], 'contract')
        require(contract['stage'] in stages, 'contract: unknown stage')
        require(contract['id'] not in contract_ids, 'duplicate contract ID')
    ...
    for contract in contracts:
        stage, ident = contract['stage'], contract['id']
        expected[f'stages/{stage}/{ident}.md'] = {
            'kind': 'contract', 'contract': ident, 'stage': stage}
    ...
    for contract in contracts:
        stage, ident = contract['stage'], contract['id']
        name = f'stages/{stage}/{ident}.md'
        contains(name, [f'stages/{stage}.md', 'README.md'])
        require(re.findall(r'^# (.+)$', docs[name], re.M) == [contract['title']],
                f'{name}: contract title does not match manifest')
    for i, s in enumerate(stages):
        name = f'stages/{s}.md'
        for h in ['Input', 'Operation and owner', 'Output and failure contract']:
            section(docs[name], h, name)
        detailed = [f"stages/{s}/{c['id']}.md" for c in contracts if c['stage'] == s]
        if detailed:
            exact(name, 'Detailed contracts', detailed)
        else:
            require('## Detailed contracts' not in docs[name], f'{name}: unlisted contracts')
