<!-- spec: {"contract": "binding-requests", "kind": "contract", "stage": "03-requests"} -->
...
`A: Target` ties the adapter to its policy and rendering-payload types (sections [5](../04-values/target-operations.md)–[6](../04-values/target-interface.md)). The method borrows the registry and adapter, consumes requests, and builds private working state. The returned `Generation` owns retained plans and payloads. Unsupported requests appear in its report; invalid input or invariant failures return `PlanningError` (section [9](../06-retain/generation.md)). Rendering and I/O follow planning.
...
Conversion planning takes a `TypeView` and a direction, represented by `Crossing`. A `TypeView` is a read-only handle retaining an exact Rust type reading and the immutable Flat model in which it is interpreted; the [Flat V2 project](../02-flat/flat-inspection.md#type-readings-and-type-views) defines its lookup and navigation API. Frontends and adapters supply source descriptions, not cache keys:
