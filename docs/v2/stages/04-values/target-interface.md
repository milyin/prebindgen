<!-- spec: {"contract": "target-interface", "kind": "contract", "stage": "04-values"} -->
...
`TargetSupport<Answer>` means a ready description, a specific unsupported reason, or a fatal planning error; its definition is in section [9](../06-retain/generation.md). `BoundarySpec` describes native argument/result placement (section [8](../05-boundary/plans.md)). `SurfaceSpec` describes a public foreign declaration and its dependencies (section [9](../06-retain/generation.md)).
