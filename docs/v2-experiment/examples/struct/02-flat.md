...
The two views stay valid after the original Flat handle is dropped. `&Stamp` is a distinct type and requires explicit referent navigation. No wrapper stripping or parsing key text is allowed. Scalar fields are the only field-type variant specified by this first experiment.
