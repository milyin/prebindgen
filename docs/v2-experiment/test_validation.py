...
class StructureTests(unittest.TestCase):
    def setUp(self):
        ...
        self.root = Path(self.tmp.name) / 'spec'
        shutil.copytree(Path(__file__).resolve().parent, self.root,
                        ignore=shutil.ignore_patterns('__pycache__'))
        (self.root.parent / 'v2').mkdir()
        (self.root.parent / 'v2/README.md').write_text('# Existing proposal\n')
    ...
    def test_valid_applicable_paths_skip_struct_boundary(self):
        # External proposal link is retained relative to the experiment directory.
        self.assertFalse((self.root / 'examples/struct/05-boundary.md').exists())
        self.assertEqual(validate(self.root), (39, 7, 2, 13))
