...
An adapter may produce more beside it, and what else is its own decision. JniGen
writes the Kotlin that calls into the generated Rust, so both sides of that
boundary come out of one run. Cbindgen writes only the Rust, leaving the C
...
need and in what order — inner ones first, so a row's parts are ready before the
row that names them — and asks the adapter for a **converter** for each, in that
order.
...
keeps it. There is one per row, so every site that picks that row calls the same
one.
...
— it walks them to work out which crossings the binding actually reaches, and so
which converters have to exist.
...
The writer then goes over the declared items in name order and calls the adapter
back once per kind — `on_function`, `on_struct`, `on_enum`, `on_const` — handing
over the model element and taking a `TokenStream`. It parses that as Rust items
and keeps them all. Nothing else is checked: not that a function comes out, not
that it is one item, not that it mentions the item declared. JniGen's constant
hook returns two, a getter and an alias.
...
arm, one argument of a callback. That is what makes a fragment per row and a
