...
both stated in the same small vocabulary. These chapters are that vocabulary
and the machinery immediately around it.
...
wire values, direction, crossing, callback type, site, part, shape, the table,
recipe, fragment, plan.
...
directions, the crossing that pairs one with a Rust type, its key, and the name
a recipe is filed under.
